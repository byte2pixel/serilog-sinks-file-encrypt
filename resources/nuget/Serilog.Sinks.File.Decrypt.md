...
// Load your RSA private key (XML, PEM, and passphrase-encrypted PKCS#8 PEM are supported)
...
> [!NOTE]
> Private keys generated by the v6.0.0+ CLI are **passphrase-encrypted PKCS#8 PEM** by default. Pass the passphrase as the extra constructor argument — `new LocalKeyProvider("my-app-key-2026", privateKey, passphrase)` or `new LocalKeyProvider(keyMap, passphrase)` — and keep the passphrase out of source code (environment variable, secret store, etc.). Unencrypted XML and PEM keys work without a passphrase.
...
string oldPrivateKey = File.ReadAllText("./keys/private_key_2025.pem");
string newPrivateKey = File.ReadAllText("./keys/private_key_2026.pem");
...
var keyMap = new Dictionary<string, string>
{
    { "my-app-key-2026", File.ReadAllText("private_key.pem") },
};
using LocalKeyProvider keyProvider = new LocalKeyProvider(keyMap);
...
### LocalKeyProvider
```csharp
// Single key. The key may be XML, PEM, or passphrase-encrypted PKCS#8 PEM;
// the passphrase is only required (and only used) for encrypted keys.
new LocalKeyProvider(string keyId, string privateKey)
new LocalKeyProvider(string keyId, string privateKey, string? passphrase)

// Multiple keys for rotation scenarios (key ID → private key).
// One passphrase is applied to every encrypted key in the map.
new LocalKeyProvider(Dictionary<string, string> keyMap)
new LocalKeyProvider(Dictionary<string, string> keyMap, string? passphrase)
```
